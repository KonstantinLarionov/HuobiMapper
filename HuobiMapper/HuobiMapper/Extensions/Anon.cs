...
namespace HuobiMapper
{
    ...
    public static class AnonymousType
    {
        ...
        private static readonly CustomAttributeBuilder CompilerGeneratedAttributeBuilder =
            new CustomAttributeBuilder(typeof(CompilerGeneratedAttribute).GetConstructor(Type.EmptyTypes),
                new object[0]);
        ...
        private static readonly MethodInfo StringBuilderAppendString = typeof(StringBuilder).GetMethod("Append",
            BindingFlags.Instance | BindingFlags.Public, null, new[] { typeof(string) }, null);

        private static readonly MethodInfo StringBuilderAppendObject = typeof(StringBuilder).GetMethod("Append",
        ...
        private static readonly MethodInfo EqualityComparerDefault = EqualityComparer.GetMethod("get_Default",
        ...
        private static readonly MethodInfo EqualityComparerEquals = EqualityComparer.GetMethod("Equals",
            ...
            new[] { EqualityComparerGenericArgument, EqualityComparerGenericArgument }, null);

        private static readonly MethodInfo EqualityComparerGetHashCode = EqualityComparer.GetMethod("GetHashCode",
            BindingFlags.Instance | BindingFlags.Public, null, new[] { EqualityComparerGenericArgument }, null);
            ...
            if (!GeneratedTypes.TryGetValue(fullName, out type))
            {
                ...
                lock (GeneratedTypes)
                {
                    if (!GeneratedTypes.TryGetValue(fullName, out type))
                    {
                        ...
                        string name = names.Length != 0
                            ? string.Format("<>f__AnonymousType{0}`{1}", index, names.Length)
                            : string.Format("<>f__AnonymousType{0}", index);
                        ...
                        ConstructorBuilder constructor = tb.DefineConstructor(
                            MethodAttributes.Public | MethodAttributes.HideBySig, CallingConventions.HasThis, generics);
                        ...
                        {
                            // field
                            fields[i] = tb.DefineField(string.Format("<{0}>i__Field", names[i]), generics[i],
                            ...
                            // getter
                            MethodBuilder getter = tb.DefineMethod(string.Format("get_{0}", names[i]),
                                MethodAttributes.Public | MethodAttributes.HideBySig | MethodAttributes.SpecialName,
                                CallingConventions.HasThis, generics[i], Type.EmptyTypes);
                            ...
                            PropertyBuilder property = tb.DefineProperty(names[i], PropertyAttributes.None,
                                CallingConventions.HasThis, generics[i], Type.EmptyTypes);
                        ...
                        // GetHashCode()
                        MethodBuilder getHashCode = tb.DefineMethod("GetHashCode",
                            MethodAttributes.Public | MethodAttributes.Virtual | MethodAttributes.HideBySig,
                        ...
                        for (int i = 0; i < names.Length; i++)
                        {
                            ...
                            ilgeneratorToString.Emit(OpCodes.Ldstr,
                                i == 0 ? string.Format("{{ {0} = ", names[i]) : string.Format(", {0} = ", names[i]));
